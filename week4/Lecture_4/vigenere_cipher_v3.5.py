    ...
    try:
        with open(filename) as file:
            if encrypted_filename:
                if isfile(encrypted_filename):
                    print('{} already exists, giving up.'.format(encrypted_filename))
        ...
        print('Could not open {}, giving up.'.format(filename))

def decrypt_file(key, filename, decrypted_filename = None):
    try:
        with open(filename) as file:
            if decrypted_filename:
                if isfile(decrypted_filename):
                    print('{} already exists, giving up.'.format(decrypted_filename))
...
def break_key_for_file(filename, *, nb_of_options_for_subkey = nb_of_options_for_subkey,
                                    ...
                                    fraction_of_letters = fraction_of_letters,
                                    fraction_of_words = fraction_of_words,
                                    etaoin_length = etaoin_length):
    try:
        with open(filename) as file:
            break_key(file.read(),
                      nb_of_options_for_subkey = nb_of_options_for_subkey,
                      max_key_length = max_key_length,
                      fraction_of_letters = fraction_of_letters,
                      fraction_of_words = fraction_of_words,
                      etaoin_length = etaoin_length)
...
def break_key(text, *, nb_of_options_for_subkey = nb_of_options_for_subkey,
    ...
    if not dictionary:
        ...
        try:
            ...
            print('Could not open the dictionary file {}, giving up.'.format(dictionary_filename))
            sys.exit()
    relevant_factors = []
    # Kasiski examination
    # Collecting all factors between 2 and max_key_length of the distances between
    # the starts of two consecutive occurrences of a string of length between 3 and 6. 
    for n in range(3, 6):
...
def factors_from_most_to_least_frequent(factors):
    '''
    >>> factors_from_most_to_least_frequent((2, 4, 5, 10, 2, 4, 2)) in ([2, 4, 10, 5], [2, 4, 5, 10])
    True
    '''
