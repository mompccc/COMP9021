...
class levenshtein_distance:
    ...
    # Taking paper and pope as an example,
    # since (p*aper, po*pe*) is one of the best aligned pairs,
    # entwined_aligned_pairs will eventually record it
    # (together with all other alignments of minimal cost) as
    # r*eeppa**opp: p*aper and po*pe* are processed from right to left
    # and entwined.
    ...
    def _compute_alignments(self, i, j, entwined_aligned_pairs):
        ...
        for direction in self._backtraces[i][j]:
            ...
            if direction == 'H':
                ...
                substitution_pairs = [pair + self.word_1[i - 1] + self.word_2[j - 1]
        ...
        # We replace these final segments by all complete best alignments that are now available.
    ...
    def get_aligned_pairs(self):
        ...
        [('p*aper', 'po*pe*'), ('pa*per', 'p*ope*'), ('paper', 'pope*')]
