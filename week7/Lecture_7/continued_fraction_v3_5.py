...
class ContinuedFraction:
    ...
    def _approximate_as_decimals(self, fractions, precision):
        ...
        s1 = fraction._get_precision_many_decimals(abs(fraction.numerator) % fraction.denominator * 10,
                                                   fraction.denominator, precision)
        while True:
            fraction = next(fractions)
...
class Fraction:
    ...
    def approximate_as_decimals(self, precision = 0):
        ...
        current_precision = precision
        while True:
            yield integral_part + '.' + ''.join(str(digit)
                              for digit in self._get_precision_many_decimals(
                                                         abs(self.numerator) % self.denominator * 10,
                                                         self.denominator, current_precision))
            current_precision += max(precision, 1)
