    ...
    for i in range(height):
        ...
        for j in range(width):
            ...
            node_luminance = alphabet[int((mean(tuple(image.crop((x, y, x + nb_of_pixels,
                                                                  y + nb_of_pixels)).getdata()))
                                                     - min_luminance) / luminance_range * len(alphabet))]
            print(f'\\node at ({j}, {i}) '
                  f'{{\\verbº{node_luminance}º}};', file = tex_file)
...
for file in (ascii_image_name + ext for ext in ('.aux', '.log')):
    os.remove(file)
print(f'\nProduced {ascii_image_name + ".pdf"}, '
      f'replacing blocks of {nb_of_pixels}x{nb_of_pixels} pixels by one character.')
