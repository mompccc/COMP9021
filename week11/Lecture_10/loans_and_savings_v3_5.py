# Defines a class Savings and a class Loans that both derive from a class
...
class Account:
    ...
    def __init__(self, *, interest, reference_period, term_amount, duration):
        ...
        self._unknowns = {Account.INITIAL_SUM, Account.TERM_AMOUNT, Account.FINAL_SUM, Account.DURATION}
    ...
    def set_reference_period(self, reference_period):
        if reference_period not in Account.nb_in_year:
            raise ValueError('Reference period should be one of {}'.format(list(Account.nb_in_year)))
    ...
    def check_type(self, parameter, parameter_name, valid_type):
        if not isinstance(parameter, valid_type):
            raise TypeError('{} should be of type in {}'.format(parameter_name, valid_type))
    ...
    def update(self):
        ...
        print('Annual interest:\t {:.2f}%'.format(float(self.interest * 100)))
        ...
        if isinstance(self, Loan):
            if all_known or Account.INITIAL_SUM not in self._unknowns:
                print('Sum borrowed:\t\t {:.2f}'.format(float(self.initial_sum)))
            else:
                print('Sum borrowed:\t\t Unknown')
        if all_known or Account.TERM_AMOUNT not in self._unknowns:
            if isinstance(self, Savings):
                print('Yearly deposits:\t {:.2f}'.format(float(self.term_amount)))
            else:
                print('Monthly repayments:\t {:.2f}'.format(float(self.term_amount)))
        else:
            if isinstance(self, Savings):
        ...
        if isinstance(self, Savings):
            if all_known or Account.FINAL_SUM not in self._unknowns:       
                print('Available sum:\t\t {:.2f}'.format(float(self.final_sum)))
            else:
        ...
        if Account.FINAL_SUM in self._unknowns:
            self._final_sum = (self.term_amount / self.effective_interest *
                                            ((1 + self.effective_interest) **
                                                    (self.duration + 1) - (1 + self.effective_interest)))
        elif Account.TERM_AMOUNT in self._unknowns:
            self._term_amount = (self.final_sum * self.effective_interest /
                  ((1 + self.effective_interest) ** (self.duration + 1) - (1 + self.effective_interest)))
        else:
            self._duration = log10(self.final_sum * self.effective_interest /
               self.term_amount + (1 + self.effective_interest)) / log10(1 + self.effective_interest) - 1
        return True
    ...
    def solve(self):
        ...
        if Account.INITIAL_SUM in self._unknowns:
            self._initial_sum = (-self.term_amount * ((1 + monthly_interest) **
                                                (12 * self.duration) - 1) / monthly_interest /
            ...
            self._duration = log10(self.term_amount / (monthly_interest * self.initial_sum +
                                                  self.term_amount)) / (12 * log10(1 + monthly_interest))
        return True
