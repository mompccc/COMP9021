        ...
        for i in range(len(self.scrollable_tape.cells[0]) - 1, -1, -1):
            ...
            for i in range(1, len(self.scrollable_tape.cells[1])):
                if self.scrollable_tape.bits[1][i] == 1:
                    self.scrollable_tape.tape.itemconfig(self.scrollable_tape.cells[1][i], fill = 'red',
                                                         font = ('normal', 0, 'bold'))
                    self.current_index = i
                    break
            else:
...
class ScrollableTape(tk.Frame):
    ...
    def __init__(self):
        ...
        w = (self.nb_of_cells_without_scroll + 2 * self.cell_proportion_for_endspace) * self.cell_size
    ...
    # self.max_indexes[1] and self.max_indexes[0] record  the number of these extra cells, respectively.
    ...
    def determine_right_boundary(self):
        return self.cell_size * (max(self.nb_of_cells_without_scroll,
                                     self.max_indexes[1]) + self.cell_proportion_for_endspace)
    ...
    def draw_minimal_tape(self):
        ...
        self.tape.create_line(-0.5 * self.cell_size, 0, -0.5 * self.cell_size, self.cell_size, width = 2,
    ...
    def draw_horizontal_lines(self, left_boundary, right_boundary):
        self.tape.create_line(left_boundary, 0, right_boundary - left_boundary, 0, width = 3,
                              fill = self.cell_colour)
        self.tape.create_line(left_boundary, self.cell_size, right_boundary, self.cell_size, width = 3,
                              fill = self.cell_colour)
    ...
    def add_cell_to_end(self, side):
        ...
        s = side * 2 - 1
        self.cells[side][i] = self.tape.create_text(i * s * self.cell_size, self.cell_size / 2, text = 0)
        self.lines[side][i] = self.tape.create_line(s * (i + 0.5) * self.cell_size, 0,
                                                    s * (i + 0.5) * self.cell_size, self.cell_size,
    ...
    def read_instructions(self):
        ...
        for instruction in source_instructions.splitlines():
            ...
            if (state, int(bit)) in self.instructions:
                tkinter.messagebox.showerror('Instruction error', 'More than one instruction for pair '
                                                                         '"({}, {})"'.format(state, bit))
                return False
