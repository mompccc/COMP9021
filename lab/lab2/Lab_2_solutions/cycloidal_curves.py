...
user_input = turtle.textinput('Hypotrochoid or Epitrochoid?',
                              'Provide no input for an Epitrochoid, and any input for a Hypotrochoid')
...
R = turtle.numinput('Fixed circle',
                    f'Radius R between {min_R} and {max_R}: ', minval = min_R, maxval = max_R)
R = round(R)
# max_r defined in such a way that centre of rolling circle is at most dim away from origin.
min_r, max_r = 10, dim - direction * R
r = turtle.numinput('Rolling circle',
                    f'Radius r between {min_r} and {max_r}: ', minval = min_r, maxval = max_r)
r = round(r)
# d defined in such a way that drawing point is at most dim away from origin.
if direction == 1 or r < R:
    min_d, max_d = 0, dim - R - direction * r 
else:
    min_d, max_d = 0, dim - r + R
