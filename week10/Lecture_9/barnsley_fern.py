...
class Drawing(tk.Frame):
        ...
        if fern_species == 'Custom':
            ...
            for n in range(4):
                try:
                    ...
                    self.fern_parameters_board.probas[n].config(bg = self.error_colour)
                    incorrect_input = True
                ...
                for i in range(2):
                    try:
                        ...
                        self.fern_parameters_board.translation_vectors[n][i].config(bg = self.error_colour)
                        incorrect_input = True
                    for j in range(2):
                        try:
    ...
    def draw_fern(self):
        weighted_parts_of_fern = [i for i in range(4) for _ in range(self.fern.probas[i])]
        point = 0, 0
...
class FernParametersBoard(tk.Frame):
    def __init__(self):
        ...
        for n in range(4):
            ...
            self.probas[n] = tk.Entry(self, bd = 1, width = 3)
