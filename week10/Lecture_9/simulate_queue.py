    ...
    def run_simulation(self):
        ...
        for simulation_tick in range(simulation_limit):
            ...
            if service_time == -1 and not queue.is_empty():
                service_time = queue.peek_at_front().service_time
                cumulative_waiting_time += simulation_tick - queue.peek_at_front().arrival_time
            ...
            while service_time == 0:
                cumulative_waiting_and_serving_time += simulation_tick - queue.peek_at_front().arrival_time
                queue.dequeue()
        ...
        if nb_of_customers:
            print(f'Number of customers who have joinded the queue: {round(nb_of_customers)}')
            print('Average number of customers in queue including those being served: '
                  'f{round(cumulative_queue_length / simulation_limit)}')
